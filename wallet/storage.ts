...
class LSDB extends DBInterface{
	...
	backup():void{
		...
		if(!data)
			return
		let ts = Date.now();
		this.LS.setItem("spectre-wallet-"+ts, data);
		let cache = this.getCache();
		if(cache)
			this.LS.setItem("spectre-cache-"+ts, cache);
	}
	saveWallet(data:string):void{
		this.LS.setItem("spectre-wallet", data);
	}

	getWallet():string|false{
		return this.LS.getItem("spectre-wallet");
	}

	saveCache(cache:string){
		return this.LS.setItem("spectre-cache", cache);
	}

	getCache():string|undefined{
		return this.LS.getItem("spectre-cache");
	}
}
...
if(IS_NODE){
	...
	class FileDB extends DBInterface{
		...
		constructor(opt:DBOptions={}){
			super();
			let {fileName="spectre", folder} = opt;
		...
		getHomeFolder(){
			...
			if(APPDATA) // windows
				return path.join(APPDATA, 'Spectre');

			if(process.platform == 'darwin')
				return path.join(HOME,'/Library/Application Support/Spectre/');

			return path.join(HOME, '.spectre');
		}
...
export class Storage{
	...
	constructor(opt:StorageOpts={}){
		this.logger = CreateLogger("SpectreStorage");
