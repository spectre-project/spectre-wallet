...
// @ts-ignore
const secp256k1 = spectrecore.secp256k1;//require('secp256k1-wasm');
...
export class AddressManager extends EventTargetImpl {
	constructor(HDWallet: spectrecore.HDPrivateKey, network: Network) {
		super();
		this.HDWallet = HDWallet;
		this.network = network;
	}

	private HDWallet: spectrecore.HDPrivateKey;

	network: Network;

	get all(): Record < string, spectrecore.PrivateKey > {
		return {
	...
	receiveAddress: {
		...
		current: {
			address: string;
			privateKey: spectrecore.PrivateKey
		};
		keypairs: Record < string, spectrecore.PrivateKey > ;
	...
	private deriveAddress(
		...
		//let address1 = new spectrecore.PublicKey(publicKeys.pubkey, {network:this.network}).toAddress().toString();
		...
		let address = spectrecore.Address.fromPublicKeyBuffer(xonly, this.network).toString();
