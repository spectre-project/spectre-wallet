...
import {SpectreAPI, ApiError} from './api';
...
const SompiPerSpectre = 100_000_000

// MaxSompi is the maximum transaction amount allowed in sompi.
const MaxSompi = 1_161_000_000 * SompiPerSpectre

export {spectrecore, COMPOUND_UTXO_MAX_COUNT, CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
...
class Wallet extends EventTargetImpl {
	...
	static spectrecore=spectrecore;
	...
	//spectrecore.Transaction.EstimatedStandaloneMassWithoutInputs;

	// TODO - integrate with Spectrecore-lib
	static networkTypes: Object = {
		spectre: { port: 18110, network: 'spectre', name : 'mainnet' },
		spectretest: { port: 18210, network: 'spectretest', name : 'testnet' },
		spectresim: { port: 18510, network: 'spectresim', name : 'simnet' },
		spectredev: { port: 18610, network: 'spectredev', name : 'devnet' }
	}
	...
	static SPR(v:number): string {
		return SPR(v);
	}


	static initRuntime() {
		return spectrecore.initRuntime();
	}
	...
	HDWallet: spectrecore.HDPrivateKey;
	 ...
	 * Current network.
	 */
	network: Network = 'spectre';

	api: SpectreAPI; //new SpectreAPI();
	...
	constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
		super();
		this.logger = CreateLogger('SpectreWallet');
		this.api = new SpectreAPI();
		//@ts-ignore
		...
		if (privKey && seedPhrase) {
			this.HDWallet = new spectrecore.HDPrivateKey(privKey);
			this.mnemonic = seedPhrase;
		} else {
			const temp = new Mnemonic(Mnemonic.Words.ENGLISH);
			this.mnemonic = temp.toString();
			this.HDWallet = new spectrecore.HDPrivateKey(temp.toHDPrivateKey().toString());
		}
	...
	*/
	 ...
	 * @param obj
	 * @param obj.toAddr To address in cashaddr format (e.g. spectretest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
	 * @param obj.amount Amount to send in sompis (100000000 (1e8) sompis in 1 SPR)
	...
	composeTx({
		...
		try {
			const tx: spectrecore.Transaction = new spectrecore.Transaction()
				.from(utxos)
				.to(toAddr, amount)
				.setVersion(0)
				.fee(fee)
				.change(changeAddr)
			if(!skipSign)
				tx.sign(privKeys, spectrecore.crypto.Signature.SIGHASH_ALL, 'schnorr');
	...
	validateAddress(addr:string):boolean{
		let address = new spectrecore.Address(addr);
	...
	*/
	 ...
	 * @param txParams
	 * @param txParams.toAddr To address in cashaddr format (e.g. spectretest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
	 * @param txParams.amount Amount to send in sompis (100000000 (1e8) sompis in 1 SPR)
	...
	async composeTxAndNetworkFeeInfo(txParamsArg: TxSend): Promise < TxInfo >{
		...
		this.logger.info(`tx ... amount: ${SPR(txParamsArg.amount)} user fee: ${SPR(txParamsArg.fee)} max data fee: ${SPR(txParamsArg.networkFeeMax||0)}`)
		...
		this.logger.verbose(`tx ... need data fee: ${SPR(dataFee)} total needed: ${SPR(amountRequested+dataFee)}`)
		this.logger.verbose(`tx ... available: ${SPR(amountAvailable)} in ${data.utxos.length} UTXOs`)

		if(networkFeeMax && dataFee > networkFeeMax) {
			throw new Error(`Fee max is ${networkFeeMax} but the minimum fee required for this transaction is ${SPR(dataFee)} SPR`);
		}
		...
		if(calculateNetworkFee){
			do {
				...
				this.logger.verbose(`tx ... need data fee: ${SPR(dataFee)} for ${data.utxos.length} UTXOs`);
			...
			if(networkFeeMax && txParams.fee > networkFeeMax)
				throw new Error(`Maximum network fee exceeded; need: ${SPR(dataFee)} SPR maximum is: ${SPR(networkFeeMax)} SPR`);

		}else if(dataFee > priorityFee){
			throw new Error(`Minimum fee required for this transaction is ${SPR(dataFee)} SPR`);
	...
	async buildTransaction(txParamsArg: TxSend, debug = false): Promise < BuildTxResult > {
		...
		this.logger.info(`tx ... required data fee: ${SPR(dataFee)} (${utxos.length} UTXOs)`);// (${SPR(txParamsArg.fee)}+${SPR(dataFee)})`);
		//this.logger.verbose(`tx ... final fee: ${SPR(dataFee+txParamsArg.fee)} (${SPR(txParamsArg.fee)}+${SPR(dataFee)})`);
		this.logger.info(`tx ... resulting total: ${SPR(totalAmount)}`);
		...
		const ts_3 = Date.now();
		const inputs: RPC.TransactionInput[] = tx.inputs.map((input: spectrecore.Transaction.Input) => {
			if (debug || this.loggerLevel > 0) {
		...
		const ts_4 = Date.now();
		const outputs: RPC.TransactionOutput[] = tx.outputs.map((output: spectrecore.Transaction.Output) => {
			return {
		...
		//@ ts-ignore
		//const payloadHash = spectrecore.crypto.Hash.sha256sha256(Buffer.from(payloadStr));
	...
	*/
	...
	setLogLevel(level: string) {
		...
		spectrecore.setDebugLevel(level?1:0);
	}
}
